...
This project runs in a browser during development and can also be packaged as a Windows or Debian desktop app with Electron. It is built with Node.js, Express, EJS, Prisma and MySQL, and follows the way a real jewellery shop works: update rates, manage stock, create a bill, take payment, print or share the invoice, and keep the books in sync.

The repository also includes sample PDFs for sales, customer orders, scheme receipts, URD purchases and pledge loans, plus dummy workbooks for every Excel report the ERP can export. They use realistic jewellery values and are separate from the shop database, so visitors can understand the document flows without seeing business records or API secrets.

- [Open the sample sales invoice PDF](docs/portfolio/assets/Kusum-ERP-demo-sales-invoice.pdf)
- [Browse all sample PDFs and Excel reports](docs/portfolio/assets/README.md)
- [See how the pieces fit together](docs/portfolio/architecture.md)
- [Look at the sample sale data](docs/portfolio/demo-sale.cjs)
- [See the dummy Excel report fixtures](docs/portfolio/demo-excel-reports.cjs)
- [Set up a local demo login](docs/portfolio/demo-login.md)
- [Review the sample rate card](docs/portfolio/rate-card.md)
...
The last command regenerates all sample PDFs and every portfolio Excel report in `docs/portfolio/assets/` without connecting to MySQL. To run the complete ERP, follow the MySQL setup below, copy `.env.example` to `.env`, run the migrations and start the server.

### In one sentence

> A full-stack jewellery ERP built with Electron, Express, Prisma and MySQL, covering inventory, barcode billing, GST-aware invoices, customer ledgers, cashbook reconciliation, Excel exports, label printing and opt-in WhatsApp delivery for sales invoice PDFs.

### About the sample

The samples are generated by the same PDF and Excel builders used by the ERP. They are kept separate from the shop database, rendered and checked before they are included here. WhatsApp credentials stay in local environment variables and are never committed.

## Default shop profile

Out of the box, a new installation starts with the **Kusum Jewellers** profile used by the sample documents: shop identity, address, GSTIN, phone numbers, invoice prefix, GST and HSN defaults, and label identity. That gives a new shop a usable starting point without requiring a long setup before the first screen appears.

The defaults are not fixed branding. After the database is provisioned, open **Business settings** in the ERP and replace the shop name, address, GST/PAN details, phone numbers, invoice prefix, tax defaults, signature and label settings with the new shop's details. The same saved settings flow into invoices, receipts, Excel headings/metadata, labels and the login/welcome screens. WhatsApp credentials remain environment-only and are configured separately.

The portfolio workbooks intentionally keep the default Kusum Jewellers profile so their headings, footers and metadata match the PDFs. They are illustrative files and can be regenerated whenever the sample data or document layout changes.
...
Do not commit `.env`, database backups, private shop exports or temporary work files. The only exports kept in Git are the clearly labelled dummy portfolio files under `docs/portfolio/assets/`. Configure credentials locally through environment variables.
