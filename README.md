...
The repository also includes sample PDFs for sales, customer orders, scheme receipts, URD purchases and pledge loans, plus a small Excel sales register. They use realistic jewellery values and are separate from the shop database, so visitors can understand the document flows without seeing business records or API secrets.

- [Open the sample sales invoice PDF](docs/portfolio/assets/Kusum-ERP-demo-sales-invoice.pdf)
- [Browse all sample PDFs and the Excel register](docs/portfolio/assets/README.md)
...
The last command regenerates all sample PDFs and the Excel register in `docs/portfolio/assets/` without connecting to MySQL. To run the complete ERP, follow the MySQL setup below, copy `.env.example` to `.env`, run the migrations and start the server.
...
The sample is generated by the same PDF module used by the ERP. It is kept separate from the shop database, rendered as an A4 PDF and checked before it is included here. WhatsApp credentials stay in local environment variables and are never committed.
